    ...
    def __init__(
        ...
        **kwargs,
        ...
        ecr_repository = ecr.Repository(
            self,
            "ECRRepository",
            repository_name=f"{config['stack_name_base'].lower()}-{pattern}",
        ...
        pattern_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "patterns", pattern
        )
        source_asset = s3_assets.Asset(self, "SourceAsset", path=pattern_path)

        # CodeBuild Role
        codebuild_role = iam.Role(
            self,
            "CodeBuildRole",
            role_name=f"{config['stack_name_base']}-backend-codebuild-role",
        ...
        build_project = codebuild.Project(
            ...
            project_name=f"{config['stack_name_base']}-{pattern}-build",
            description=f"Build {pattern} agent Docker image for {config['stack_name_base']}",
            role=codebuild_role,
        ...
        build_trigger_function = lambda_.Function(
            ...
            handler="utils.build_trigger_lambda.handler",
